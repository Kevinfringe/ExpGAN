...
class LandmarkLoss(nn.Module):
    ...
    def get_lm_patches(self, input):
        """
            This function is to extract the 9*9 region around landmarks
            and form them in (N * 68 * 9 * 9) tensor, since there are 68 landmarks
            detected.
        :return: (N * 68 * 9 * 9) tensor
        """
        ...
        for i in range(batch_size):
            ...
            for j in range(lm_length):
                x_cor = int(self.lm_array[i, 0, 2 + j]) if (self.lm_array[i, 0, 2 + j] < 252) else 251
                x_cor = x_cor if x_cor > 4 else 4
                y_cor = int(self.lm_array[i, 0, 2 + lm_length + j]) if self.lm_array[i, 0, 2 + lm_length + j] < 252 else 251
                y_cor = y_cor if y_cor > 4 else 4

                res[i, :, x_cor - 4: x_cor + 4, y_cor - 4 : y_cor + 4] = input[i, :, x_cor - 4: x_cor + 4, y_cor - 4:y_cor + 4]
